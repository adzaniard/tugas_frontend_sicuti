        ...
        <main class="flex-1 p-10">
            <div class="bg-white shadow rounded-lg p-8 max-w-xl mt-7 mx-auto">
                ...
                <div class="flex items-center justify-center space-x-2 mb-6">
                    ...
                    <h1 class="text-2xl font-bold text-center">Edit Kajur</h1>
                </div>
                ...
                <form action="{{ route('kajur.update', $kajur['id_kajur']) }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input name="nama_kajur" type="text" value="{{ $kajur['nama_kajur'] }}" required class="w-full border border-gray-300 rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">NIDN</label>
                        <input name="nidn" type="text" value="{{ $kajur['nidn'] }}" required class="w-full border border-gray-300 rounded px-3 py-2" />
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Nama Jurusan</label>
                        <input name="nama_jurusan" type="text" value="{{ $kajur['nama_jurusan'] }}" required class="w-full border border-gray-300 rounded px-3 py-2" />
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_user" required class="w-full border border-gray-300 rounded px-3 py-2">
                            ...
                            @foreach ($user as $usr)
                            <option value="{{ $usr['id_user'] }}" {{ $usr['id_user'] == $kajur['id_user'] ? 'selected' : '' }}>
                                {{ $usr['username'] }}
                            </option>
                            @endforeach
                        </select>
